...
parse(const std::string &input) noexcept {
  ...
  auto extract = [&](const std::string &docstring,
...
\r]+))");
  ...
  auto parse_doc = [&](openapi::OpenAPI &spec, const std::string &docstring) {
    if (auto v = extract(docstring, "title"))
      spec.info.title = *v;
    if (auto v = extract(docstring, "summary"))
      spec.info.summary = *v;
    if (auto v = extract(docstring, "version"))
      spec.info.version = *v;
    if (auto v = extract(docstring, "description"))
      spec.info.description = *v;
    if (auto v = extract(docstring, "termsOfService"))
      spec.info.termsOfService = *v;

    if (auto v = extract(docstring, "contact_name")) {
      if (!spec.info.contact.has_value())
    ...
    if (auto v = extract(docstring, "license_name")) {
      if (!spec.info.license.has_value())
        spec.info.license = openapi::License{};
    ...
    if (schemes_idx != std::string::npos) {
      if (!spec.components.has_value())
        spec.components = openapi::Components{};
      if (!spec.components->securitySchemes.has_value())
      ...
      while (std::getline(iss, line)) {
        ...
        if (std::regex_search(lcontent, m, scheme_re)) {
          ...
          if (m[5].matched && !m[5].str().empty())
            scheme.scheme = m[5].str();
          if (m[6].matched && !m[6].str().empty())
            scheme.bearerFormat = m[6].str();
          ...
          if (std::regex_search(lcontent, m, desc_re)) {
            std::string key = m[1].str();
            if (spec.components->securitySchemes->find(key) !=
                spec.components->securitySchemes->end()) {
          ...
          if (std::regex_search(lcontent, m, oid_re)) {
            ...
            if (spec.components->securitySchemes->find(key) !=
                spec.components->securitySchemes->end()) {
              (*spec.components->securitySchemes)[key].openIdConnectUrl =
                  m[2].str();
            }
          }
          ...
          if (std::regex_search(lcontent, m, oauth_re)) {
            ...
            if (spec.components->securitySchemes->find(key) !=
                spec.components->securitySchemes->end()) {
              (*spec.components->securitySchemes)[key].oauth2MetadataUrl =
                  m[2].str();
            }
          }
  ...
  auto words_begin =
      std::sregex_iterator(input.begin(), input.end(), method_regex);
  ...
  for (std::sregex_iterator i = words_begin; i != words_end; ++i) {
    ...
    if (!args_str.empty() && args_str != "const std::string&") {
      ...
      auto arg_begin =
          std::sregex_iterator(args_str.begin(), args_str.end(), arg_regex);
    ...
    if (std::regex_search(comments_block, summary_match,
    ...
    if (std::regex_search(comments_block, op_desc_match,
    ...
    auto res_begin = std::sregex_iterator(comments_block.begin(),
    ...
    auto sec_begin = std::sregex_iterator(comments_block.begin(),
    ...
    auto rc_begin = std::sregex_iterator(comments_block.begin(),
                                         comments_block.end(), resp_content_re);
    auto rc_end = std::sregex_iterator();
    if (rc_begin != rc_end) {
      if (!op.responses.has_value())
        op.responses = std::map<std::string, openapi::Response>{};
