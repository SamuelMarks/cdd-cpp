...
openapi::OpenAPI parse(const std::string &input) {
  ...
  for (const auto &fn : ast.functions) {
    ...
    if (std::regex_search(fn.body, url_match, url_regex)) {
      ...
      if (method == "GET") spec.paths->at(path).get = op;
      else if (method == "POST") spec.paths->at(path).post = op;
      else if (method == "PUT") spec.paths->at(path).put = op;
      else if (method == "DELETE") spec.paths->at(path).delete_op = op;
      else if (method == "PATCH") spec.paths->at(path).patch = op;
    }
  }
