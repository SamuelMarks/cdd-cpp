...
void emit_Info(utils::JsonWriter &jw, const Info &obj) noexcept {
  ...
  if (obj.contact.has_value()) {
    jw.key("contact");
    emit_Contact(jw, obj.contact.operator*());
  }
  if (obj.license.has_value()) {
    jw.key("license");
    emit_License(jw, obj.license.operator*());
  }
...
void emit_ServerVariable(utils::JsonWriter &jw,
  ...
  if (obj.enum_values.has_value() && !obj.enum_values->empty()) {
    jw.key("enum");
    jw.start_array();
    for (const auto &item : obj.enum_values.operator*())
      jw.value(item);
    jw.end_array();
  }
...
void emit_Server(utils::JsonWriter &jw, const Server &obj) noexcept {
  ...
  if (obj.variables.has_value() && !obj.variables->empty()) {
    jw.key("variables");
    jw.start_object();
    for (const auto &[k, v] : obj.variables.operator*()) {
      jw.key(k);
...
void emit_Tag(utils::JsonWriter &jw, const Tag &obj) noexcept {
  ...
  if (obj.externalDocs.has_value()) {
    jw.key("externalDocs");
    emit_ExternalDocumentation(jw, obj.externalDocs.operator*());
  }
  jw.end_object();
}
void emit_Discriminator(utils::JsonWriter &jw,
  ...
  if (obj.mapping.has_value() && !obj.mapping->empty()) {
    jw.key("mapping");
    jw.start_object();
    for (const auto &[k, v] : obj.mapping.operator*()) {
      jw.key_value(k, v);
    }
    jw.end_object();
  }
...
void emit_XML(utils::JsonWriter &jw, const XML &obj) noexcept {
  ...
  if (obj.attribute.has_value())
    jw.key_value("attribute", obj.attribute.operator*());
  if (obj.wrapped.has_value())
    jw.key_value("wrapped", obj.wrapped.operator*());
  jw.end_object();
}
...
void emit_Schema(utils::JsonWriter &jw, const Schema &obj) noexcept {
  ...
  if (obj.ref.has_value()) {
    jw.key_value("$ref", obj.ref.operator*().ref);
    jw.key_optional("summary", obj.ref.operator*().summary);
    jw.key_optional("description", obj.ref.operator*().description);
  }
  if (obj.discriminator.has_value()) {
    jw.key("discriminator");
    emit_Discriminator(jw, obj.discriminator.operator*());
  }
  if (obj.xml.has_value()) {
    jw.key("xml");
    emit_XML(jw, obj.xml.operator*());
  }
...
void emit_Encoding(utils::JsonWriter &jw, const Encoding &obj) noexcept {
  ...
  if (obj.explode.has_value())
    jw.key_value("explode", obj.explode.operator*());
  if (obj.allowReserved.has_value())
    jw.key_value("allowReserved", obj.allowReserved.operator*());
...
void emit_Header(utils::JsonWriter &jw, const Header &obj) noexcept {
  ...
  if (obj.content.has_value() && !obj.content->empty()) {
    jw.key("content");
    jw.start_object();
    for (const auto &[k, v] : obj.content.operator*()) {
      jw.key(k);
      emit_MediaType(jw, v);
    }
    jw.end_object();
...
void emit_Link(utils::JsonWriter &jw, const Link &obj) noexcept {
  ...
  if (obj.parameters.has_value() && !obj.parameters->empty()) {
    jw.key("parameters");
    jw.start_object();
    for (const auto &[k, v] : obj.parameters.operator*())
      jw.key_value(k, v);
    jw.end_object();
  }
  ...
  if (obj.server.has_value()) {
    jw.key("server");
    emit_Server(jw, obj.server.operator*());
  }
  if (obj.ref.has_value()) {
    jw.key_value("$ref", obj.ref.operator*().ref);
    jw.key_optional("summary", obj.ref.operator*().summary);
...
void emit_Response(utils::JsonWriter &jw, const Response &obj) noexcept {
  ...
  if (obj.headers.has_value() && !obj.headers->empty()) {
    jw.key("headers");
    jw.start_object();
    for (const auto &[k, v] : obj.headers.operator*()) {
      jw.key(k);
      emit_Header(jw, v);
    }
    jw.end_object();
  ...
  if (obj.links.has_value() && !obj.links->empty()) {
    jw.key("links");
    jw.start_object();
    for (const auto &[k, v] : obj.links.operator*()) {
      jw.key(k);
      emit_Link(jw, v);
    }
    jw.end_object();
...
void emit_OAuthFlow(utils::JsonWriter &jw, const OAuthFlow &obj) noexcept {
  ...
  if (obj.scopes.has_value() && !obj.scopes->empty()) {
    jw.key("scopes");
    jw.start_object();
    for (const auto &[k, v] : obj.scopes.operator*())
      jw.key_value(k, v);
    jw.end_object();
  }
  jw.end_object();
...
void emit_OAuthFlows(utils::JsonWriter &jw, const OAuthFlows &obj) noexcept {
  ...
  if (obj.implicit.has_value()) {
    jw.key("implicit");
    emit_OAuthFlow(jw, obj.implicit.operator*());
  }
  if (obj.password.has_value()) {
    jw.key("password");
    emit_OAuthFlow(jw, obj.password.operator*());
  }
  if (obj.clientCredentials.has_value()) {
    jw.key("clientCredentials");
    emit_OAuthFlow(jw, obj.clientCredentials.operator*());
  }
  if (obj.authorizationCode.has_value()) {
    jw.key("authorizationCode");
    emit_OAuthFlow(jw, obj.authorizationCode.operator*());
  }
  if (obj.deviceAuthorization.has_value()) {
    jw.key("deviceAuthorization");
    emit_OAuthFlow(jw, obj.deviceAuthorization.operator*());
  }
  jw.end_object();
}
...
void emit_SecurityScheme(utils::JsonWriter &jw,
  ...
  if (obj.flows.has_value()) {
    jw.key("flows");
    emit_OAuthFlows(jw, obj.flows.operator*());
  }
...
void emit_Operation(utils::JsonWriter &jw, const Operation &obj) noexcept {
  ...
  if (obj.tags.has_value() && !obj.tags->empty()) {
    jw.key("tags");
    jw.start_array();
    for (const auto &item : obj.tags.operator*())
      jw.value(item);
    jw.end_array();
  }
  jw.key_optional("summary", obj.summary);
  ...
  if (obj.parameters.has_value() && !obj.parameters->empty()) {
    jw.key("parameters");
    jw.start_array();
    for (const auto &item : obj.parameters.operator*())
      emit_Parameter(jw, item);
    jw.end_array();
  }
  if (obj.requestBody.has_value()) {
    jw.key("requestBody");
    emit_RequestBody(jw, obj.requestBody.operator*());
  }
  if (obj.responses.has_value() && !obj.responses->empty()) {
    jw.key("responses");
    jw.start_object();
    for (const auto &[k, v] : obj.responses.operator*()) {
      jw.key(k);
      emit_Response(jw, v);
    }
    jw.end_object();
  ...
  if (obj.security.has_value() && !obj.security->empty()) {
    jw.key("security");
    jw.start_array();
    for (const auto &req : obj.security.operator*()) {
      jw.start_object();
  ...
  if (obj.servers.has_value() && !obj.servers->empty()) {
    jw.key("servers");
    jw.start_array();
    for (const auto &item : obj.servers.operator*())
      emit_Server(jw, item);
    jw.end_array();
  }
  jw.end_object();
...
void emit_PathItem(utils::JsonWriter &jw, const PathItem &obj) noexcept {
  ...
  if (obj.get.has_value()) {
    jw.key("get");
    emit_Operation(jw, obj.get.operator*());
  }
  if (obj.put.has_value()) {
    jw.key("put");
    emit_Operation(jw, obj.put.operator*());
  }
  if (obj.post.has_value()) {
    jw.key("post");
    emit_Operation(jw, obj.post.operator*());
  }
  if (obj.delete_op.has_value()) {
    jw.key("delete");
    emit_Operation(jw, obj.delete_op.operator*());
  }
  if (obj.options.has_value()) {
    jw.key("options");
    emit_Operation(jw, obj.options.operator*());
  }
  if (obj.head.has_value()) {
    jw.key("head");
    emit_Operation(jw, obj.head.operator*());
  }
  if (obj.patch.has_value()) {
    jw.key("patch");
    emit_Operation(jw, obj.patch.operator*());
  }
  if (obj.trace.has_value()) {
    jw.key("trace");
    emit_Operation(jw, obj.trace.operator*());
  }
  if (obj.query.has_value()) {
    jw.key("query");
    emit_Operation(jw, obj.query.operator*());
  }
  if (obj.servers.has_value() && !obj.servers->empty()) {
    jw.key("servers");
    jw.start_array();
...
void emit_Components(utils::JsonWriter &jw, const Components &obj) noexcept {
  ...
  if (obj.schemas.has_value() && !obj.schemas->empty()) {
    jw.key("schemas");
    jw.start_object();
    for (const auto &[k, v] : obj.schemas.operator*()) {
      jw.key(k);
      emit_Schema(jw, v);
    }
    jw.end_object();
  ...
  if (obj.requestBodies.has_value() && !obj.requestBodies->empty()) {
    ...
    jw.start_object();
    for (const auto &[k, v] : obj.requestBodies.operator*()) {
      jw.key(k);
  ...
  if (obj.securitySchemes.has_value() && !obj.securitySchemes->empty()) {
    ...
    jw.start_object();
    for (const auto &[k, v] : obj.securitySchemes.operator*()) {
      jw.key(k);
  ...
  if (obj.pathItems.has_value() && !obj.pathItems->empty()) {
    jw.key("pathItems");
    jw.start_object();
    for (const auto &[k, v] : obj.pathItems.operator*()) {
      jw.key(k);
      emit_PathItem(jw, v);
    }
    jw.end_object();
...
void emit_OpenAPI(utils::JsonWriter &jw, const OpenAPI &obj) noexcept {
  ...
  if (obj.paths.has_value() && !obj.paths->empty()) {
    jw.key("paths");
    jw.start_object();
    for (const auto &[k, v] : obj.paths.operator*()) {
      jw.key(k);
      emit_PathItem(jw, v);
    }
    jw.end_object();
  ...
  if (obj.webhooks.has_value() && !obj.webhooks->empty()) {
    jw.key("webhooks");
    jw.start_object();
    for (const auto &[k, v] : obj.webhooks.operator*()) {
      jw.key(k);
      emit_PathItem(jw, v);
    }
    jw.end_object();
  ...
  if (obj.components.has_value()) {
    jw.key("components");
    emit_Components(jw, obj.components.operator*());
  }
  if (obj.security.has_value() && !obj.security->empty()) {
    jw.key("security");
    jw.start_array();
