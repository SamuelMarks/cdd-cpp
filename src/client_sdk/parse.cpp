...
\s]+)\s+\[([^\r
\]]*)\])");
      auto sec_begin = std::sregex_iterator(fn.docstring.begin(),
                                            fn.docstring.end(), sec_re);
      ...
      auto rc_begin = std::sregex_iterator(fn.docstring.begin(),
                                           fn.docstring.end(), resp_content_re);
      ...
      for (const auto &arg : fn.params) {
        if (arg.name == "body") {
          ...
          openapi::Parameter p;
          p.name = arg.name;
          // Simple heuristic: if the path contains {arg.name}, it's a path
          // param, else query.
          if (path.find("<" + arg.name + ">") != std::string::npos ||
              path.find("{" + arg.name + "}") != std::string::npos) {
            p.in = "path";
          } else {
            p.in = "query";
          }
